<template>
  <div class="min-h-screen bg-gray-50 font-sans text-gray-900">
    ...
    <section class="mx-auto max-w-7xl px-4 sm:px-8 py-6">
      <div
        class="relative overflow-hidden rounded-3xl bg-[#f4f0ec] px-6 sm:px-12 py-10 sm:py-12 flex items-center justify-between shadow-sm"
      >
        <div class="max-w-lg z-10">
          <span
            class="inline-block bg-white px-3 py-1 rounded-full text-xs font-bold text-gray-700 mb-4 shadow-sm"
            >#Big Fashion Sale</span
          >
          <h1
            class="mb-3 text-3xl sm:text-4xl font-extrabold tracking-tight text-[#3b3632]"
          >
            Limited Time Offer!<br />Up to
            <span class="text-red-600">50% OFF!</span>
          </h1>
          <p class="mb-6 text-sm text-gray-600 font-medium">
            Redefine Your Everyday Style with sustainable, premium daily
            essentials.
          </p>
          <button
            class="rounded-full bg-[#3b3632] px-8 py-3 text-sm font-semibold text-white transition hover:bg-black shadow-md"
          >
            Shop Collection
          </button>
        </div>
        ...
        <div class="hidden md:flex gap-4 items-center z-10">
          <div
            class="h-52 w-36 bg-white rounded-2xl shadow-xl p-2 -rotate-6 overflow-hidden transition hover:rotate-0"
          >
            <img
          ...
          <div
            class="h-60 w-40 bg-white rounded-2xl shadow-2xl p-2 rotate-[4deg] overflow-hidden transition hover:rotate-0"
          >
            <img
    ...
    <!-- Category Circles -->
    <div
      class="flex items-center justify-center space-x-6 overflow-x-auto py-4 px-4"
    >
      <div
    ...
    <section class="mx-auto max-w-7xl px-4 sm:px-8 py-8 mb-16">
      <div class="mb-6 flex flex-wrap items-center justify-between gap-4">
        <h2 class="text-2xl font-bold">
          <span v-if="productStore.searchQuery"
            >Search Results for "{{ productStore.searchQuery }}"</span
          >
          <span v-else-if="productStore.selectedCategory">{{
      ...
      <div v-else-if="displayCategories.length > 0">
        ...
        >
          <h2
            class="text-2xl font-bold text-[#3b3632] mb-6 flex items-center gap-3"
          >
            {{ category.name }}
            <span
              class="text-xs font-medium text-gray-500 bg-gray-200 px-3 py-1 rounded-full"
            >
          ...
          <div class="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-5">
            ...
            >
              <div>
                <div
                  class="relative mb-4 flex h-48 items-center justify-center overflow-hidden"
                >
                  <img
                    :src="getImageUrl(product.image)"
                    :alt="product.name"
                    class="h-full w-full object-cover rounded-xl transition group-hover:scale-105"
                  />
                </div>
                ...
                <div class="mt-2 flex items-baseline gap-2">
                  <span class="font-bold text-gray-900"
                    >GH₵{{ product.price?.toFixed(2) }}</span
                  >
                  <span class="text-xs text-gray-400 line-through"
                    >GH₵{{ (product.price * 1.3).toFixed(2) }}</span
                  >
                </div>
              </div>
      ...
      <div
        v-else
        class="text-center py-12 text-gray-500 bg-white rounded-2xl border border-gray-100 shadow-sm"
      >
        No products found. Adjust your search or add items from the Admin
        dashboard!
      </div>
    </section>
  </div>
